...
# -*- coding: utf-8 -*-
"""
vis_prism_demo.py — PRISM Demo 视频生成器（新版布局，无坐标框）

布局（2×5 GridSpec）：
  行0: [RGB + 2D骨骼 (cols 0-1)] | [LiDAR GT (cols 2-4, 宽) ← 右侧第1个]
  行1: [Depth         (cols 0-1)] | [mmWave] [PRISM Output] [GT Skeleton] ← 下面3个

右侧 4 个面板均不显示 3D 坐标框，只显示数据内容。

用法（参数与 vis_demo.py 完全一致）：
  python vis_prism_demo.py --mmfi_start 28 --mmfi_end 46 --mist_start 47 --mist_end 66
"""
...
def _no_box_3d(ax, title, axlims, elev, azim):
    """3D 轴仅显示数据内容，不显示坐标框。"""
    ax.set_axis_off()
...
def _draw_depth(ax, depth_m, title='Depth'):
    ...
    ax.imshow(masked_c, cmap='plasma_r', vmin=1.0, vmax=4.5,
...
def make_animation(frames, save_path, fps=8, elev=10, azim=0):
    """
    新版 2×5 布局（无坐标框）：
      行0: RGB+2D骨骼 (cols 0-1) | LiDAR GT (cols 2-4, 宽)  ← 右侧第1个
      行1: Depth      (cols 0-1) | mmWave | PRISM | Skeleton ← 下面3个
    """
    ...
    ax_lidar = fig.add_subplot(gs[0, 2:], projection='3d')   # 右侧第1个
    ax_depth = fig.add_subplot(gs[1, :2])
    ax_mm    = fig.add_subplot(gs[1, 2],  projection='3d')   # 下面第1
    ax_gen   = fig.add_subplot(gs[1, 3],  projection='3d')   # 下面第2
    ax_sk    = fig.add_subplot(gs[1, 4],  projection='3d')   # 下面第3
    ...
    def update(i):
        ...
        # ── (0, 2-4) LiDAR GT（右侧第1个，无坐标框）────────────
        ax_lidar.cla()
        _no_box_3d(ax_lidar, 'LiDAR GT', axlims_li, elev, azim)
        li = fr['lidar_vis']
        if len(li) > 0:
            ax_lidar.scatter(li[:,0], li[:,1], li[:,2],
        ...
        # ── (1, 2) mmWave（下面第1，无坐标框）──────────────────
        ax_mm.cla()
        _no_box_3d(ax_mm, 'mmWave', axlims_mm, elev, azim)
        mm = fr['mmwave_vis']
        if len(mm) > 0:
        ...
        # ── (1, 3) PRISM Output（下面第2，无坐标框）────────────
        ax_gen.cla()
        _no_box_3d(ax_gen, 'PRISM Output', axlims_li, elev, azim)
        gen = fr['gen_vis']
        ...
        # ── (1, 4) GT Skeleton（下面第3，无坐标框）─────────────
        ax_sk.cla()
        _no_box_3d(ax_sk, 'GT Skeleton', axlims_sk, elev, azim)
...
def main():
    pa = argparse.ArgumentParser('PRISM Demo 视频生成器（新版布局）')
    ...
    pa.add_argument('--elev',       type=float, default=10)
    pa.add_argument('--azim',       type=float, default=0)
    ...
    print(f'\n[保存] {save_path}')
    make_animation(frames, save_path, fps=args.fps, elev=args.elev, azim=args.azim)
    print('完成。')
