...
class EMA(Optimizer):
    def __init__(self, opt, ema_decay):
        ...
        # 补全新版 PyTorch (>=2.0) Optimizer 期望的 hook 注册表
        # EMA 未调用 super().__init__()，需手动补充，否则 state_dict() 报错
        if not hasattr(self, '_optimizer_state_dict_pre_hooks'):
            self._optimizer_state_dict_pre_hooks = {}
        if not hasattr(self, '_optimizer_state_dict_post_hooks'):
            self._optimizer_state_dict_post_hooks = {}
        if not hasattr(self, '_optimizer_load_state_dict_pre_hooks'):
            self._optimizer_load_state_dict_pre_hooks = {}
        if not hasattr(self, '_optimizer_load_state_dict_post_hooks'):
    ...
    def step(self, *args, **kwargs):
        ...
        # ── 第一遍：收集所有 param_group 的参数（修复多 group 时的 bug）──
