...
% ── Title ───────────────────────────────────────────────────
\title{PRISM: Human Point Cloud Reconstruction via \\
...
% ────────────────────────────────────────────────────────────
\begin{abstract}
mmWave radar enables all-weather, non-contact human sensing, but its sparse output
(50--150 reflections per frame) severely limits downstream body analysis.
Existing point cloud completion methods target dense inputs ($>$2048 pts) from
rigid objects and fail at the combination of extreme sparsity and non-rigid
topological variability of human bodies.
We present \textbf{PRISM} (\textbf{P}oint cloud \textbf{R}econstruction v\textbf{I}a
\textbf{S}keleton-guided diffusion from \textbf{M}mWave radar), a conditional
latent diffusion framework for human point cloud completion from sparse mmWave radar.
PRISM operates along two parallel paths.
Path~\textcircled{1} refines noisy depth-camera skeletons to the radar coordinate
frame via a cross-modal attention module (SkelKD), then encodes the sparse radar
input into a structured latent representation via a latent compressor~\cite{ldt}
($8\!\times\!120$ tokens).
Path~\textcircled{2} extracts a geometric coarse prior with GeoAE (128 pts)
and jointly injects skeleton topology (GCN, 17 tokens), coarse geometry (128 tokens),
and action class label into a Dynamic Transformer that denoises the latent code under
a VP-SDE framework.
On MM-Fi, PRISM significantly outperforms point cloud completion and generative
baselines on MMD-CD, COV-CD, and 1-NN-CD while producing temporally consistent
human point cloud sequences.
We further contribute OccluRadar-Human, the first dataset with paired
complete/occluded mmWave captures and frame-level 3-D skeleton ground truth.
\end{abstract}
...
\begin{itemize}[leftmargin=1.5em,itemsep=1pt]
  ...
  \item \textbf{Latent Compressor (adapted from LDT~\cite{ldt}):} we adapt the
        LDT latent compressor to handle variable-length sparse inputs as few as
        50 points by replacing FPS-based downsampling with a LocalGrouper, and
        add ActNorm for stable VAE training, yielding a fixed-size latent
        $Z_0\!\in\!\Rbb^{B\times8\times120}$ decoupled from input point count.
...
\begin{figure}[t]
  ...
  \fbox{\parbox[c][7.2cm][c]{0.96\linewidth}{%
    ...
    $\hat{P} \xrightarrow{\text{LC Enc}}$ $Z_0 \in\Rbb^{B\times8\times120}$
    \quad
    ...
    \hat{Z}_0
    \xrightarrow{\text{LC Dec}}X_f\!\in\!\Rbb^{B\times256\times3}$
  }}
  \caption{
    ...
    sequence, yielding $\hat{Z}_0$ decoded by the latent compressor into the
    final point cloud $X_f$.
  }
  \label{fig:arch}
\end{figure}
...
The latent compressor encodes the sparse point cloud into the clean latent
...
latent compressor decoder maps to $X_f\!\in\!\Rbb^{B\times256\times3}$.
...
\subsection{Latent Compressor}
\label{sec:compressor}

Following LDT~\cite{ldt}, we employ a latent compressor that encodes a
variable-length sparse point cloud into a fixed-size structured latent
$Z_0\!\in\!\Rbb^{B\times8\times120}$ and decodes latent tokens back to a dense
point cloud.
Compared with the original LDT compressor (FPS + single-scale attention
autoencoder for dense inputs), we make two adaptations: (i) replace
FPS-based downsampling with a LocalGrouper that handles as few as 50 radar
points without failure, and (ii) insert ActNorm layers for numerically stable
VAE training on noisy radar inputs.
...
It is jointly trained with the LDT stage.
...
optimized jointly with the LDT score-matching loss.
...
Starting from the clean latent $Z_0$ from the latent compressor encoder, a
...
Latent Compressor: 6 hierarchical levels, $z_\text{dim}\!=\!20$,
...
Stage~2 (LC): Adam, lr $5\!\times\!10^{-5}$, batch 8, 2000 ep.
...
\begin{table}[t]
  \centering
  \caption{
    \textbf{Point cloud completion quality on MM-Fi.}
  ...
  \begin{tabular}{llccccc}
    ...
    \midrule
    PCN$^*$~\cite{pcn}            & 3DV'18      & Compl. & ---  & ---  & ---  & ---  \\
    PoinTr$^*$~\cite{pointr}      & ICCV'21     & Compl. & ---  & ---  & ---  & ---  \\
    SnowflakeNet$^*$~\cite{snowflake} & ICCV'21  & Compl. & ---  & ---  & ---  & ---  \\
    LAKe-Net$^*$~\cite{lakenet}   & CVPR'22     & Compl. & ---  & ---  & ---  & ---  \\
    LION$^*$~\cite{lion}          & NeurIPS'22  & Gen.   & ---  & ---  & ---  & ---  \\
    TIGER$^*$~\cite{tiger}        & CVPR'24     & Gen.   & ---  & ---  & ---  & ---  \\
    \midrule
    \rowcolor{green!8}
    \textbf{PRISM (ours)}  & ---  & Gen.
      & \todo{} & \todo{} & \todo{} & \todo{} \\
    \midrule
    LiDAR GT (Oracle)             & ---  & ---    & ---  & 1.000 & ---  & 0.000 \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:main} compares PRISM against six baselines.
MMD-CD measures distributional fidelity; COV-CD measures coverage;
1-NN-CD measures discriminability; CD measures per-sample reconstruction accuracy.
Baseline results will be filled upon experiment completion (\todo{fill}).
...
\begin{figure}[t]
  ...
  \fbox{\parbox[c][7.6cm][c]{0.96\linewidth}{%
    ...
    Expected pattern: PCN*/PoinTr* → average human shape, missing limb details;\\
    LION*/TIGER* → fails under extreme sparsity (no skeleton prior);\\
    PRISM → topologically correct, action-specific pose, close to GT
  }}
  \caption{
    \textbf{Qualitative completion comparison.}
    General completion methods produce average body shapes and miss limb details.
    LION$^*$ and TIGER$^*$ fail to handle extreme input sparsity without a
    structural prior.
...
(e.g., a bowing torso lean, a raised waving arm) while baselines converge to
an average standing posture.
...
\begin{table}[t]
  ...
  \caption{
    ...
    Full = complete PRISM; each variant removes or replaces one module.
    \todo{Fill values after experiments.}
  }
  \label{tab:ablation}
  \small
  \setlength{\tabcolsep}{5pt}
  \begin{tabular}{lcccc}
    ...
    Full PRISM (ours)                       & ---  & ---  & ---  & ---  \\
    \midrule
    \emph{w/o} SkelKD (raw skeleton)          & ---  & ---  & ---  & ---  \\
    \emph{w/o} Skeleton GCN                   & ---  & ---  & ---  & ---  \\
    \emph{w/o} GeoAE (coarse geom.)  & ---  & ---  & ---  & ---  \\
    \emph{w/o} Action class label             & ---  & ---  & ---  & ---  \\
    \emph{w/o} All conditions (uncond.)       & ---  & ---  & ---  & ---  \\
    GCN $\to$ Flatten-MLP (no topology)       & ---  & ---  & ---  & ---  \\
    GeoAE $\to$ FPS on raw mmWave       & ---  & ---  & ---  & ---  \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{SkelKD (A1).}
Raw depth-camera skeletons carry $\sim$\todo{X}\,mm systematic coordinate offset,
misaligning the condition signal; SkelKD removes this offset.

\textbf{Dual-path condition (A2).}
Removing the GCN most degrades distal joints (wrists, ankles, head) where
kinematic-chain topology propagation is essential.
Removing GeoAE loses the geometric anchor, causing shapes to diverge
from the observed radar point cloud.

\textbf{Topology-aware GCN (A3).}
A flatten-MLP discards kinematic chain structure, producing incorrect limb
proportions and relative joint positioning.

\textbf{GeoAE vs.\ FPS (A4).}
At $N\!\sim\!50$, FPS produces 32 heavily overlapping centers with zero tokens
for occluded joints; GeoAE produces complete, stable coarse geometry.
...
\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
  ...
  \item \textbf{Hardware:} Single NVIDIA RTX 3090 (24\,GB).\\
        ...
        Stage~1 (SkelKD, 5000 ep): $\sim$\todo{X}\,h;\;
        Stage~2 (LC, 2000 ep): $\sim$\todo{X}\,h;\;
        Stage~3 (LDT, 5000 ep): $\sim$\todo{X}\,h.
\end{itemize}
