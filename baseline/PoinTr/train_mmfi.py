"""
train_mmfi.py — PoinTr baseline on MM-Fi
==========================================
Usage:
    cd baseline/PoinTr
    CUDA_VISIBLE_DEVICES=2 python train_mmfi.py -c cfgs/pointr_mmfi.yaml
"""
...
from utils.loss_utils import pointr_loss, calc_cd
...
def set_folders(args):
    model_name = 'PoinTr-%s' % timetmp.strftime('%m%d_%H%M', timetmp.localtime())
...
def train(args):
    ...
    net = PoinTr(
        ...
        num_per_token  = args.num_per_token,
        feat_dim       = args.feat_dim,
        num_heads      = args.num_heads,
        num_enc_layers = args.num_enc_layers,
        num_dec_layers = args.num_dec_layers,
    ).cuda()
    logging.info('PoinTr params: %d' % sum(p.numel() for p in net.parameters()))
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        with tqdm(train_loader) as t:
            for data in t:
                ...
                coarse, fine = net(mm)
                loss, lc, lf = pointr_loss(coarse, fine, gt, alpha=args.alpha)
                loss.backward()
                optimizer.step()
